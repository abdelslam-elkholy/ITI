...
const fillingArr = (num) => {
  ...
  document.write(
    `<p style="color: red; font-size: 1.5rem"> sum of 3 values <span style="black: red; font-size: 1rem">${
      arr[0]
    } + ${arr[1]} + ${arr[2]} = ${arr[0] + arr[1] + arr[2]} </span></p>`
  );
  document.write(
    `<p style="color: red; font-size: 1.5rem"> multipication of 3 values <span style="black: red; font-size: 1rem">${
      arr[0]
    } * ${arr[1]} * ${arr[2]} = ${arr[0] * arr[1] * arr[2]} </span></p>`
  );

  document.write(
    `<p style="color: red; font-size: 1.5rem"> dividtion of 3 values <span style="black: red; font-size: 1rem">${
      arr[0]
    } / ${arr[1]} / ${arr[2]} = ${arr[0] / arr[1] / arr[2]} </span></p>`
  );
